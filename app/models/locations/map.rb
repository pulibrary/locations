module Locations
  class Map
    ...
    def hours_location
      @hours_location ||= set_hours_location(self.holding_location.locations_hours_location_id)
    end

    def is_valid?
      return true if !self.holding_location.nil? && !self.bibrec.nil?
      false
    end

    def on_reserve?
      return true if self.closed_stack_reserves.include? self.loc
      false
    end
    ...
    def set_hours_location(hours_location_id)
